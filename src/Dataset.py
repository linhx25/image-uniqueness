...
class AirbnbDataLoader(DataLoader):
    def __init__(self, root: str, batch_size=1, shuffle=True, sampler=None, drop_last=True):
    ...
    def _get_slices(self):
        ...
        if not self.drop_last:
            for i, slc in enumerate(self.batch_slices):
                last_idx = slc[-1].stop
                self.batch_slices[i].extend([
                    slice(last_idx, self.dt_len[i])
                ])
        return batch_slices
    ...
    def __getitem__(self, idx:int):
        ...
        images, ids = ds["images"], ds["IDs"] 
        return (
            images[idx_image].reshape(224, 224, 3).astype('uint8'),
            ids[idx_image]
        )
